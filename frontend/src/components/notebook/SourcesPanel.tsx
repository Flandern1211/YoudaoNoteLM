...
export default function SourcesPanel() {
  ...
  const pendingReadyAudios = notebook.sources.filter(
    s => s.type === 'audio' && s.previewId && s.status === 'ready' && s.content
  );
  ...
  const handleImportSelected = async () => {
    ...
    try {
      ...
      // 清除选中状态，但不关闭搜索面板
      setSelectedResults(new Set());
      // 后端已创建 pending 状态的 Source，store 会自动刷新列表并轮询状态
    } catch (err) {
  ...
  const handleViewSource = async (source: Source) => {
    ...
    if (source.type === 'audio' && source.previewId) {
      ...
      <div className="h-full flex flex-col bg-bg-secondary/30">
        ...
        <div className="flex-shrink-0 p-4 border-t border-border flex gap-2">
          ...
          {audioPreview.previewId && !audioTranscribing && (
            <Button
              size="sm"
              disabled={audioConfirming}
              onClick={async () => {
                if (!currentNotebookId) return;
                setAudioConfirming(true);
                try {
                  await confirmAudio(audioPreview.previewId, currentNotebookId, audioPreviewContent);
          ...
          {filteredSources.map((source) => {
            ...
            return (
              <motion.div key={source.id} layout initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}
                ...
                <div className={cn('flex-1 min-w-0', isLoading ? 'cursor-default' : 'cursor-pointer')} onClick={() => { if (!isLoading && !isError) handleViewSource(source); }}>
                  <div className="flex items-center gap-1.5">
                    <div className={cn('p-1 rounded', isError ? 'bg-error/10 text-error' : '', !isError && source.type === 'file' && 'bg-blue-500/10 text-blue-400', !isError && source.type === 'url' && 'bg-teal/10 text-teal', !isError && source.type === 'audio' && 'bg-purple-500/10 text-purple-400', !isError && source.type === 'search' && 'bg-orange-500/10 text-orange-400', !isError && source.type === 'youdao' && 'bg-green-500/10 text-green-400')}>
                      {isLoading ? <Loader2 size={12} className="animate-spin" /> : isError ? <AlertCircle size={12} /> : <Icon size={12} />}
                    </div>
                    {editingId === source.id ? (
                  ...
                  <div className="flex items-center gap-2 mt-0.5 ml-5">
                    {isLoading && <span className="text-[10px] text-accent animate-pulse">导入中...</span>}
                    {isError && (
                      <span
                    ...
                    {!isLoading && !isError && source.size !== undefined && <span className="text-[10px] text-text-muted">{formatFileSize(source.size)}</span>}
                ...
                {/* Actions: loading 状态显示中止按钮，其他状态显示菜单 */}
                {isLoading ? (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
