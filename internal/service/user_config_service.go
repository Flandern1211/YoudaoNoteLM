...
func (s *userConfigService) ListLLMConfigs(userID uint) ([]*entity.UserConfig, error) {
	config, err := s.configRepo.FindByUserAndType(userID, "llm")
	if err != nil {
		return nil, err
	}
	if config == nil {
...
func (s *userConfigService) CreateLLMConfig(userID uint, config *entity.UserConfig) error {
	config.UserID = userID
	config.ConfigType = "llm"
	if config.ExtraConfig == "" {
		config.ExtraConfig = "{}"
	}

	// 检查是否已经存在相同类型的配置（包括已删除的记录）
	existing, err := s.configRepo.FindByUserAndTypeIncludingDeleted(userID, "llm")
	if err != nil {
		return err
	}
...
func (s *userConfigService) UpdateLLMConfig(id uint, config *entity.UserConfig) error {
	...
	config.ConfigType = "llm"
	// 确保 extra_config 不为空（MySQL JSON 类型不接受空字符串）
	if config.ExtraConfig == "" {
		config.ExtraConfig = "{}"
	}
	...
	if err != nil {
		logger.Error("更新配置失败", zap.Uint("id", id), zap.Error(err))
		return err
	}
	// 清除 ConfigService 中的用户配置缓存
	s.configSvc.ClearUserConfigCache(existing.UserID, "llm")
	return nil
}

func (s *userConfigService) DeleteLLMConfig(id uint) error {
	// 先查到配置以获取 userID，用于清除缓存
	existing, err := s.configRepo.FindByID(id)
	if err != nil {
		return err
	}
